...
Recorder.addEventHandler('sendKeys', 'keydown', function (event) {    
    if (event.target.tagName) {
        ...
        var type = event.target.type;  
        debugger;
        //added to handle hover event on web element
        if (event.keyCode==70 && mouseOverTarget) {            
            this.record("focus", this.locatorBuilders.buildAll(mouseOverTarget), "");
        }
...
// SELENIUM-240 : 2013x: handle the global actions menu : START
// 13x enhancements: added click event handler code to manage
function clickEventhandler (recorderObj, event)
{
    if (event.button == 0 && !preventClick && event.isTrusted) {
        if (!preventClickTwice) {
            var top = event.pageY,
            ...
            var target = event.target;
            recorderObj.record("click", this.locatorBuilders.buildAll(event.target), '');
            var arrayTest = this.locatorBuilders.buildAll(event.target);
            preventClickTwice = true;
        }
        setTimeout(function () { preventClickTwice = false; }, 30);
...
Recorder.addEventHandler('dragAndDrop', 'mousedown', function (event) {
    ...
    // SELENIUM-240 : 2013x: handle the global actions menu : START
    var currentNodeName = event.target.nodeName.toLowerCase();
    var currentNodeID = event.target.attributes["menuuid"];

    var parentNodeName = event.target.parentNode.nodeName.toLowerCase();
    var parentNodeID = event.target.parentNode.attributes["menuuid"];

    // Commenting below Code as Click event issue 
    //    identified in 13x GA OOTB
    //    is resolved in 13x FP1440 OOTB
    //if((currentNodeName == "li" && currentNodeID) || (parentNodeName == "li" && parentNodeID)) // Call Click event code
    //{
    //    clickEventhandler(this, event);
    //}
    // SELENIUM-240 : 2013x: handle the global actions menu : END
    //else { // CALL orginal cal for drag and drop
        var self = this;
