...
function getCommandTargetDatalist(tr) {
	return tr.getElementsByTagName("td")[1].getElementsByTagName("datalist")[0];
}

function getCommandSGSTargetDatalist(tr) {
	return tr.getElementsByTagName("td")[1].getElementsByTagName("sgs")[0];
}
...
function setColor(index, state) {
	if (typeof (index) == "string") {
		...
		if (state == "fail") {
			document.getElementById("result-failures").textContent = $('.test-case-title.'
				+ state).length;
		} else {
			document.getElementById("result-runs").textContent = $('.test-case-title.'
				+ state).length
				- $('.test-case-title.success.fail').length;
		}
		// KAT-END
	} else {
...
function setCaseScrollTop(testCase) {
	...
	else if ($(".case_list").height() + $(".case_list").scrollTop() - 60 < testCase.offsetTop
		- $(".case_list").offset().top)
		$(".case_list").animate(
			{
				scrollTop: testCase.offsetTop
					- $(".case_list").offset().top
					- ($(".case_list").height() - 60)
			}, 10);
}
	...
	try {
		if (i % 2 == 1) {
			while (i <= end) {
				...
				if (!node.className || node.className == "odd"
					|| node.className == "even") {
					node.className = "odd";
				}
				i = parseInt(i) + 1;
...
function attachEvent(start, end) {
	...
	if(closebutton)
	{
		closebutton.addEventListener("click", function (event){
			$("#command-toolbar").addClass("hidden");
	...
	for (var i = start; i <= end; ++i) {
		...
		node
			.addEventListener(
				"click",
				function (event) {
					$("#command-toolbar").removeClass("hidden");
					$("#closeButton").css("display","unset");
					
					// use jquery's API to add and remove class property
					if (firstSelectedTrId == undefined
						&& $(".selectedRecord").length > 0) {
						firstSelectedTrId = parseInt($(".selectedRecord")[0].id
							.substring(8));
					}

					if (!event.ctrlKey && !event.shiftKey) {
						$('#records-grid .selectedRecord').removeClass(
							'selectedRecord');
					...
					if (event.shiftKey) {
						if (firstSelectedTrId != undefined) {
							let thisSelectedTrId = parseInt($(this)[0].id
								.substring(8));
							$('#records-grid .selectedRecord')
								.removeClass('selectedRecord');
					...
					document.getElementById("command-target").value = getCommandTarget(
						ref, true);
					var targetList = ref.getElementsByTagName("td")[1]
					...
					assignChildNodes(document
						.getElementById("target-dropdown"),
						targetList, false);
					assignChildNodes(document
						.getElementById("command-target-list"), ref
							.getElementsByTagName("td")[1]
							.getElementsByTagName("datalist")[0], true,
						true);
...
function getSelectedCase() {
	if (document.getElementById("testCase-grid").getElementsByClassName(
		"selectedCase")) {
		return document.getElementById("testCase-grid").getElementsByClassName(
			"selectedCase")[0];
	} else {
		return null;
	}
...
function addChildTextNode(parentNode, name, value) {
	var option = document.createElement(name);
...
function addCommand(command_name, command_target_array, command_value, auto,
	...
	var s_suite = getSelectedSuite(), s_case = getSelectedCase();
	if (!s_suite || !s_case) {
	...
	//var commandXML = buildCommand(command_name, command_target_array,
	//		command_value);
	...
	for (var i = 0; i < 3; ++i) {
		...
		if (i == 0) {
			...
			// use textNode to avoid tac's tag problem (textNode's content will
			// be pure text, does not be parsed as html)
			div_hidden.appendChild(document
				.createTextNode(command_target_array[0][0]));
		} else {
	...
	var sgsDiv_hidden = document.createElement("div");
	sgsDiv_hidden.style = "display:none;";
	var sgsTarget = document.createElement("sgs");
	for (var m = 0; m < command_target_array.length; ++m) {

		var expressionType = command_target_array[m][1];
		if (expressionType && expressionType.startsWith("sgs:")) {
			addChildTextNode(sgsTarget, "name", command_target_array[m][1]);
			addChildTextNode(sgsTarget, "value", command_target_array[m][0]);
		} else {
			addChildTextNode(targets, "option", command_target_array[m][0]);
		}
	}

	//store frame names in hidden tag
	/*if (command_name && command_name.toLowerCase() == "selectframe" && command_target_array.length >= 2) {
		addChildTextNode(sgsTarget, "name", "name");
		addChildTextNode(sgsTarget, "value", command_target_array[1][0]);
	}*/

	new_record.getElementsByTagName("td")[1].appendChild(targets);
	sgsDiv_hidden.appendChild(sgsTarget);
	new_record.getElementsByTagName("td")[1].appendChild(sgsDiv_hidden);
	...
	if (selected_ID) {
		if (auto) {
			document.getElementById(selected_ID).parentNode.insertBefore(
				new_record, document.getElementById(selected_ID));
			selected_ID = parseInt(selected_ID.split("-")[1]);
		} else {
			document.getElementById(selected_ID).parentNode.insertBefore(
				new_record,
				document.getElementById(selected_ID).nextSibling);
		...
		if (insertCommand) {
			document.getElementById("records-grid").insertBefore(new_record,
				getRecordsArray()[getRecordsNum() - 2]);
		} else {
	...
	for (var k = 0; k < 3; ++k) {
		...
		if (k == 0) {
			...
			// some target is not a pure string, so we need to change the type
			// to string
		...
		getTdShowValueNode(new_record, k).appendChild(
	...
	if (s_case) {
		sideex_testCase[s_case.id].records = document
			.getElementById("records-grid").innerHTML;
	}
}
...
function addCommandBeforeLastCommand(command_name, command_target_array,
	command_value) {
